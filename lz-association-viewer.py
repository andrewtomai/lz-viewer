...
def check_header(filename):
	#specify the command to be sent to stdout
	command = "tabix " + filename + " -H"
	#send command to stdout
	p =  subprocess.Popen(command, shell=True, stdout=subprocess.PIPE)
	#take in the text that was output
	text = p.stdout.read()
	retcode = p.wait()
...
def gather_data(filename, names):
	...
	#specify the command to be sent to stdout
	command = "tabix " + filename + " 11:193154-193775"	
	#send to stdout, and take in the text that was output
	p = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE)
	text = p.stdout.read()
	retcode = p.wait()
...
def api():
	...
	data = gather_data(filename, header)
